...
SampleNT <- readRDS(file.path(proj.dir, "data", "SampleNT.RDS"))
SampleNT_short <- substr(x = SampleNT, start = 1, stop = 12)
SampleTP <- readRDS(file.path(proj.dir, "data", "SampleTP.RDS"))
...
DEGanalysis <- function(stages, title,
  ...
  for(s in stages) {
    ...
    SampleNT_Stage <- filter(Stage_NT, Stage_NT$bcr_patient_barcode %in% SampleNT_short)
    ...
    SampleTP_Stage <- filter(Stage_TP, Stage_TP$bcr_patient_barcode %in% SampleTP_short)
  ...
  assign("queryDown", 
         GDCquery(project = CancerProject,
                  ...
                  experimental.strategy = "RNA-Seq",
                  legacy = TRUE))
  saveRDS(queryDown, file.path(proj.dir, "data", paste0("GDCquery_", title, ".RDS")))
  # Download samples
  ...
  saveRDS(dataPrep, file.path(proj.dir, "data", paste0("GDCprepare_", title, ".RDS")))
  ...
  saveRDS(dataPreProc, file.path(proj.dir, "data", paste0("dataPreProc_", title, ".RDS")))
  # Samples normalisation
  flog.debug("Perform normalization")
  dataNorm <- TCGAanalyze_Normalization(tabDF = dataPreProc,
                                        geneInfo = geneInfo,
                                        method = Norm_method)
  saveRDS(dataNorm, file.path(proj.dir, "data", paste0("dataNorm_", title, ".RDS")))
  # Data filtering
  ...
  saveRDS(patients,
          file.path(proj.dir, "data", paste0("patients_", title,".RDS")))
  ...
  saveRDS(dataDEGs,
          file.path(proj.dir, "data", paste0("dataDEG_", title,".RDS")))
}
